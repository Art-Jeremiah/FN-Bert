...
def training(opt):
    ...
    if opt.eval_model:
        ...
        dev_raw_examples = processor.read_json(os.path.join(opt.raw_data_dir, 'final_to_use.json'))
        dev_examples = processor.get_examples(dev_raw_examples, 'final_to_use')
...
def final_evaluation(opt):
    ...
    dev_raw_examples = processor.read_json(os.path.join(opt.raw_data_dir, 'final_to_use.json'))
    dev_examples = processor.get_examples(dev_raw_examples, 'final')
    
    dev_features, dev_callback_info = convert_examples_to_features(opt.task_type, dev_examples,
                                                                   opt.max_seq_len, opt.bert_dir, ent2id)

    dev_dataset = NERDataset(opt.task_type, dev_features, 'final', use_type_embed=opt.use_type_embed)
    ...
    model, device = load_model_and_parallel(model, opt.gpu_ids[0],
                                                    ckpt_path="./out/bert_wwm_wd_crf/checkpoint-1020/model.pt")
    
    # model, device = load_model_and_parallel(model, opt.gpu_ids[0],
    #                                                 ckpt_path="./out/roberta_wwm_large_wd_crf/checkpoint-408/model.pt")
...
if __name__ == '__main__':
    ...
    assert args.mode in ['train', 'stack','final'], 'mode mismatch'
